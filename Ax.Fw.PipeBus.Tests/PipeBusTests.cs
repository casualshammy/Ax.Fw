...
public class PipeBusTests
{
    ...
    public async Task StressTestClientServer(int _num)
    {
        ...
        try
        {
            var server = PipeBusServer.Run(lifetime, ThreadPoolScheduler.Instance, pipeName, false);
            var client0 = PipeBusClient.Run(lifetime, ThreadPoolScheduler.Instance, pipeName);
            var client1 = PipeBusClient.Run(lifetime, ThreadPoolScheduler.Instance, pipeName);
    ...
    public async Task StressTestClientServerRunAsync(int _num)
    {
        ...
        try
        {
            var server = await PipeBusServer.RunAsync(lifetime, ThreadPoolScheduler.Instance, pipeName, false);
            var client0 = await PipeBusClient.RunAsync(lifetime, ThreadPoolScheduler.Instance, pipeName);
            var client1 = await PipeBusClient.RunAsync(lifetime, ThreadPoolScheduler.Instance, pipeName);
            ...
            lifetime.DisposeOnCompleted(
                client0
                    .OfReqRes<SimpleMsgReq, SimpleMsgRes>(async _msg =>
                    {
                        itemBag.Add(_msg.Code);
                        return new SimpleMsgRes(_msg.Code + 1);
                    }));
            ...
            for (int _ = 0; _ < 1000; _++)
            {
                ...
                var i = _;
                var swi = Stopwatch.StartNew();
                var result = await client1.PostReqResOrDefaultAsync<SimpleMsgReq, SimpleMsgRes>(new SimpleMsgReq(i), TimeSpan.FromSeconds(5), lifetime.Token);
                if (result == null)
                    p_output.WriteLine($"Error0: {i}; {itemBag.Contains(i)}");
    ...
    public async Task ServerAsClientTest()
    {
        ...
        try
        {
            var server = await PipeBusServer.RunAsync(asyncLifetime, ThreadPoolScheduler.Instance, pipeName, true);
            var client = await PipeBusClient.RunAsync(asyncLifetime, ThreadPoolScheduler.Instance, pipeName);
