...
public static class DirectoryInfoExtensions
{
  ...
  {
    List<string> files = Directory.GetFiles(_directoryInfo.FullName, "*.*", SearchOption.AllDirectories).OrderBy(_ => _).ToList();
    using (MD5 md5 = MD5.Create())
    {
      for (int i = 0; i < files.Count; i++)
      {
        string file = files[i];
        string relativePath = file[(_directoryInfo.FullName.Length + 1)..];
      ...
      if (md5.Hash == null)
        throw new InvalidDataException("Can't calculate md5: hash is empty!");

      return BitConverter.ToString(md5.Hash).Replace("-", "");
    }
  }

  public static void DirectoryCopy(this DirectoryInfo _sourceDirectory, string _destDirName, bool _copySubDirs)
  {
